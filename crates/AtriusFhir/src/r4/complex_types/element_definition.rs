...
#[fhir_invariant(key="eld-11", severity="error", human="Binding can only be present for coded elements, string, and uri", expr="binding.empty() or type.code.empty() or type.select((code = 'code') or (code = 'Coding') or (code='CodeableConcept') or (code = 'Quantity') or (code = 'string') or (code = 'uri')).exists()", path="ElementDefinition")]
...
pub struct ElementDefinition {
    ...
    #[fhir_invariant(key="eld-17", severity="error", human="targetProfile is only allowed if the type is Reference or canonical", expr="(code='Reference' or code = 'canonical') or targetProfile.empty()", path="ElementDefinition.type")]
    ...
    /// ## Conditions
    /// Used when: eld-11
    #[fhir_invariant(key="eld-12", severity="error", human="ValueSet SHALL start with http:// or https:// or urn:", expr="valueSet.exists() implies (valueSet.startsWith('http:') or valueSet.startsWith('https') or valueSet.startsWith('urn:'))", path="ElementDefinition.binding")]
