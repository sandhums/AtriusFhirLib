...
#[fhir_invariant(key="ref-1", severity="error", human="SHALL have a contained resource if a local reference is provided", expr="reference.startsWith('#').not() or (reference.substring(1).trace('url') in %rootResource.contained.id.trace('ids'))", path="Reference")]
