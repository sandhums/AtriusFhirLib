...
#[fhir_invariant(key="ait-1", severity="error", human="AllergyIntolerance.clinicalStatus SHALL be present if verificationStatus is not entered-in-error.", expr="(verificationStatus.exists() and verificationStatus.coding.where(system='http://terminology.hl7.org/CodeSystem/allergyintolerance-verification' and code='entered-in-error').exists().not()) implies clinicalStatus.exists()", path="AllergyIntolerance")]
#[fhir_invariant(key="ait-2", severity="error", human="AllergyIntolerance.clinicalStatus SHALL NOT be present if verification Status is entered-in-error", expr="(verificationStatus.coding.where(system='http://terminology.hl7.org/CodeSystem/allergyintolerance-verification' and code='entered-in-error').exists()) implies clinicalStatus.exists().not()", path="AllergyIntolerance")]
...
pub struct AllergyIntolerance {
    ...
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children unless an empty Parameters resource", expr="hasValue() or (children().count() > id.count()) or $this is Parameters", path="AllergyIntolerance.reaction")]
