...
pub struct QuestionnaireResponse {
    ...
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children unless an empty Parameters resource", expr="hasValue() or (children().count() > id.count()) or $this is Parameters", path="QuestionnaireResponse.item")]
    #[fhir_invariant(key="qrs-1", severity="error", human="Nested item can't be beneath both item and answer", expr="(answer.exists() and item.exists()).not()", path="QuestionnaireResponse.item")]
...
pub struct QuestionnaireResponseItem {
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children unless an empty Parameters resource (error)
    ///   Expression: `hasValue() or (children().count() > id.count()) or $this is Parameters`
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children unless an empty Parameters resource", expr="hasValue() or (children().count() > id.count()) or $this is Parameters", path="QuestionnaireResponse.item.answer")]
