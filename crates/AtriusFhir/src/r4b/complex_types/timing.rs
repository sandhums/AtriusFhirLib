...
pub struct Timing {
    ...
    #[fhir_invariant(key="ext-1", severity="error", human="Must have either extensions or value[x], not both", expr="extension.exists() != value.exists()", path="Timing.extension")]
    ...
    #[fhir_invariant(key="ext-1", severity="error", human="Must have either extensions or value[x], not both", expr="extension.exists() != value.exists()", path="Timing.modifierExtension")]
    ...
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children unless an empty Parameters resource", expr="hasValue() or (children().count() > id.count()) or $this is Parameters", path="Timing.repeat")]
    ...
    #[fhir_invariant(key="tim-9", severity="error", human="If there's an offset, there must be a when (and not C, CM, CD, CV)", expr="offset.empty() or (when.exists() and ((when in ('C' | 'CM' | 'CD' | 'CV')).not()))", path="Timing.repeat")]
...
pub struct TimingRepeat {
    ...
    #[fhir_invariant(key="ext-1", severity="error", human="Must have either extensions or value[x], not both", expr="extension.exists() != value.exists()", path="Timing.repeat.extension")]
    pub extension: Option<Vec<Extension>>,
