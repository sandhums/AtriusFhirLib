// @generated by AtriusValueSetGenerator
// DO NOT EDIT MANUALLY

#![allow(non_camel_case_types)]
#![allow(non_snake_case)]
#![allow(clippy::upper_case_acronyms)]

use super::super::super::{Code, CodeableConcept, Coding, Element, Uri};

use super::super::super::string::String as FhirString;

///FHIR ValueSet: NHINPurposeOfUse
///Canonical URL: http://hl7.org/fhir/ValueSet/nhin-purposeofuse
///Version: 5.0.0
///Title: NHIN PurposeOfUse
///Status: active
///This value set is suitable for use with the provenance resource. It is derived from, but not compatible with, the HL7 v3 Purpose of use Code system.
///Compose includes 27 explicit concept codes
///Includes systems:
///- http://healthit.gov/nhin/purposeofuse
pub struct NHINPurposeOfUse;
impl NHINPurposeOfUse {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/nhin-purposeofuse";
    pub const HAS_NONLOCAL_RULES: bool = false;
    pub fn version() -> Option<&'static str> {
        Some("5.0.0")
    }
    pub fn include_systems() -> &'static [&'static str] {
        &["http://healthit.gov/nhin/purposeofuse"]
    }
    pub fn include_value_sets() -> &'static [&'static str] {
        &[]
    }
    /// Systems that are included as whole CodeSystems but are not locally enumerable.
    ///
    /// If this is non-empty, callers should use a terminology server for definitive validation.
    pub fn include_whole_systems() -> &'static [&'static str] {
        &[]
    }
    /// Returns true only when this ValueSet can be treated as fully locally checkable.
    ///
    /// If `HAS_NONLOCAL_RULES` is true (filters, include.valueSet, or non-enumerated whole-system includes),
    /// local checks are best-effort and may require a terminology server.
    pub fn is_locally_enumerated() -> bool {
        !Self::HAS_NONLOCAL_RULES
            && (!Self::expansion_pairs().is_empty() || !Self::include_pairs().is_empty())
    }
    fn include_pairs() -> &'static [(&'static str, &'static str)] {
        &[
            ("http://healthit.gov/nhin/purposeofuse", "TREATMENT"),
            ("http://healthit.gov/nhin/purposeofuse", "PAYMENT"),
            ("http://healthit.gov/nhin/purposeofuse", "OPERATIONS"),
            ("http://healthit.gov/nhin/purposeofuse", "SYSADMIN"),
            ("http://healthit.gov/nhin/purposeofuse", "FRAUD"),
            ("http://healthit.gov/nhin/purposeofuse", "PSYCHOTHERAPY"),
            ("http://healthit.gov/nhin/purposeofuse", "TRAINING"),
            ("http://healthit.gov/nhin/purposeofuse", "LEGAL"),
            ("http://healthit.gov/nhin/purposeofuse", "MARKETING"),
            ("http://healthit.gov/nhin/purposeofuse", "DIRECTORY"),
            ("http://healthit.gov/nhin/purposeofuse", "FAMILY"),
            ("http://healthit.gov/nhin/purposeofuse", "PRESENT"),
            ("http://healthit.gov/nhin/purposeofuse", "EMERGENCY"),
            ("http://healthit.gov/nhin/purposeofuse", "DISASTER"),
            ("http://healthit.gov/nhin/purposeofuse", "PUBLICHEALTH"),
            ("http://healthit.gov/nhin/purposeofuse", "ABUSE"),
            ("http://healthit.gov/nhin/purposeofuse", "OVERSIGHT"),
            ("http://healthit.gov/nhin/purposeofuse", "JUDICIAL"),
            ("http://healthit.gov/nhin/purposeofuse", "LAW"),
            ("http://healthit.gov/nhin/purposeofuse", "DECEASED"),
            ("http://healthit.gov/nhin/purposeofuse", "DONATION"),
            ("http://healthit.gov/nhin/purposeofuse", "RESEARCH"),
            ("http://healthit.gov/nhin/purposeofuse", "THREAT"),
            ("http://healthit.gov/nhin/purposeofuse", "GOVERNMENT"),
            ("http://healthit.gov/nhin/purposeofuse", "WORKERSCOMP"),
            ("http://healthit.gov/nhin/purposeofuse", "COVERAGE"),
            ("http://healthit.gov/nhin/purposeofuse", "REQUEST"),
        ]
    }
    fn include_entries() -> &'static [(
        &'static str,
        &'static str,
        Option<&'static str>,
        Option<&'static str>,
    )] {
        &[
            (
                "http://healthit.gov/nhin/purposeofuse",
                "ABUSE",
                Some("Abuse"),
                Some("Disclosures about victims of abuse, neglect or domestic violence."),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "COVERAGE",
                Some("Coverage"),
                Some("Disclosures for insurance or disability coverage determination"),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "DECEASED",
                Some("Deceased"),
                Some("Uses and disclosures about decedents."),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "DIRECTORY",
                Some("Directory"),
                Some("Use and disclosure for facility directories"),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "DISASTER",
                Some("Disaster"),
                Some("Use and disclosures for disaster relief purposes."),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "DONATION",
                Some("Donation"),
                Some(
                    "Uses and disclosures for cadaveric organ,  eye or tissue donation purposes",
                ),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "EMERGENCY",
                Some("Emergency"),
                Some(
                    "Permission cannot practicably be provided because of the individual's incapacity or an emergency.",
                ),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "FAMILY",
                Some("Family"),
                Some(
                    "Disclose to a family member, other relative, or a close personal friend of the individual",
                ),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "FRAUD",
                Some("Fraud"),
                Some("Fraud detection"),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "GOVERNMENT",
                Some("Government"),
                Some("Uses and disclosures for specialized government functions."),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "JUDICIAL",
                Some("Judicial"),
                Some("Disclosures for judicial and administrative proceedings."),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "LAW",
                Some("Law Enforcement"),
                Some("Disclosures for law enforcement purposes."),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "LEGAL",
                Some("Legal"),
                Some(
                    "Use or disclosure by the covered entity to defend itself in a legal action",
                ),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "MARKETING",
                Some("Marketing"),
                Some("Marketing"),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "OPERATIONS",
                Some("Operations"),
                Some("Healthcare Operations"),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "OVERSIGHT",
                Some("Oversight"),
                Some("Uses and disclosures for health oversight activities."),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "PAYMENT",
                Some("Payment"),
                Some("Payment"),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "PRESENT",
                Some("Present"),
                Some("Uses and disclosures with the individual present."),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "PSYCHOTHERAPY",
                Some("Psychotherapy"),
                Some("Use or disclosure of Psychotherapy Notes"),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "PUBLICHEALTH",
                Some("Public Health"),
                Some("Uses and disclosures for public health activities."),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "REQUEST",
                Some("Request"),
                Some("Request of the Individual"),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "RESEARCH",
                Some("Research"),
                Some("Uses and disclosures for research purposes."),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "SYSADMIN",
                Some("Sysadmin"),
                Some("System Administration"),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "THREAT",
                Some("Threat"),
                Some(
                    "Uses and disclosures to avert a serious threat to health or safety.",
                ),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "TRAINING",
                Some("Training"),
                Some(
                    "Use or disclosure by the covered entity for its own training programs",
                ),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "TREATMENT",
                Some("Treatment"),
                Some("Treatment"),
            ),
            (
                "http://healthit.gov/nhin/purposeofuse",
                "WORKERSCOMP",
                Some("Worker's Comp"),
                Some("Disclosures for workers' compensation."),
            ),
        ]
    }
    /// Return the preferred display text for a (system, code) pair, if present in this ValueSet's explicit concepts.
    pub fn display(system: &str, code: &str) -> Option<&'static str> {
        for (s, c, d, _def) in Self::include_entries() {
            if *s == system && *c == code {
                return *d;
            }
        }
        None
    }
    /// Return the ValueSet concept definition for a (system, code) pair, if present.
    ///
    /// In `valuesets.json` this often comes from the extension:
    /// `http://hl7.org/fhir/StructureDefinition/valueset-concept-definition`.
    pub fn definition(system: &str, code: &str) -> Option<&'static str> {
        for (s, c, _d, def) in Self::include_entries() {
            if *s == system && *c == code {
                return *def;
            }
        }
        None
    }
    /// Create a minimal `Coding` from a (system, code) pair.
    ///
    /// If this ValueSet provides an explicit display for the code, it will be populated.
    pub fn to_coding(system: &str, code: &str) -> Coding {
        let sys_uri: Uri = Element {
            id: None,
            extension: None,
            value: Some(system.to_string()),
        };
        let code_el: Code = Element {
            id: None,
            extension: None,
            value: Some(code.to_string()),
        };
        let display = Self::display(system, code)
            .map(|s| FhirString {
                id: None,
                extension: None,
                value: Some(s.to_string()),
            });
        Coding {
            id: None,
            extension: None,
            system: Some(sys_uri),
            version: Self::version()
                .map(|v| FhirString {
                    id: None,
                    extension: None,
                    value: Some(v.to_string()),
                }),
            code: Some(code_el),
            display,
            user_selected: None,
        }
    }
    fn exclude_pairs() -> &'static [(&'static str, &'static str)] {
        &[]
    }
    fn expansion_pairs() -> &'static [(&'static str, &'static str)] {
        &[]
    }
    /// Best-effort local validation for a `Coding` against this ValueSet.
    ///
    /// Rules:
    /// - Explicit excludes (compose.exclude.concept) always deny.
    /// - If expansion.contains exists, it is treated as an explicit allow-list.
    /// - Else if compose.include.concept exists, it is treated as an explicit allow-list.
    /// - Else: fall back to whole-system validation when possible (generated CodeSystem enums / RGB grammar).
    ///
    /// Note: terminology filters and include.valueSet references are NOT evaluated locally.
    pub fn contains_coding(coding: &Coding) -> bool {
        let system = match &coding.system {
            Some(s) => s.value.as_deref().unwrap_or(""),
            None => return false,
        };
        let code = match &coding.code {
            Some(c) => c.value.as_deref().unwrap_or(""),
            None => return false,
        };
        if Self::exclude_pairs().iter().any(|(s, c)| *s == system && *c == code) {
            return false;
        }
        let exp = Self::expansion_pairs();
        if !exp.is_empty() {
            return exp.iter().any(|(s, c)| *s == system && *c == code);
        }
        let inc = Self::include_pairs();
        if !inc.is_empty() {
            return inc.iter().any(|(s, c)| *s == system && *c == code);
        }
        match system {
            "http://healthit.gov/nhin/purposeofuse" => {
                super::super::code_systems::nhin_purposeofuse::NHINPurposeOfUse::try_from_code(
                        code,
                    )
                    .is_ok()
            }
            _ => false,
        }
    }
    /// Best-effort local validation for a `CodeableConcept` against this ValueSet.
    ///
    /// Returns true if any `coding` entry is contained in the ValueSet.
    pub fn contains_codeable_concept(cc: &CodeableConcept) -> bool {
        match cc.coding.as_ref() {
            Some(codings) => codings.iter().any(|c| Self::contains_coding(c)),
            None => false,
        }
    }
    /// Convenience: validate a FHIR `code` primitive when the binding is on `code`.
    ///
    /// This only works reliably when the ValueSet includes exactly one system.
    pub fn contains_code(code: &Code) -> bool {
        let code_str = match code.value.as_deref() {
            Some(v) => v,
            None => return false,
        };
        let systems = Self::include_systems();
        if systems.len() != 1 {
            return false;
        }
        let system: Uri = Element {
            id: None,
            extension: None,
            value: Some(systems[0].to_string()),
        };
        let code: Code = Element {
            id: None,
            extension: None,
            value: Some(code_str.to_string()),
        };
        let coding = Coding {
            id: None,
            extension: None,
            system: Some(system),
            version: None,
            code: Some(code),
            display: None,
            user_selected: None,
        };
        Self::contains_coding(&coding)
    }
    /// Create a minimal `CodeableConcept` (single coding) from a (system, code) pair.
    pub fn to_codeable_concept(system: &str, code: &str) -> CodeableConcept {
        let coding = Self::to_coding(system, code);
        CodeableConcept {
            id: None,
            extension: None,
            coding: Some(vec![coding]),
            text: None,
        }
    }
}
impl super::super::bindings::ValueSetMembership<Code> for NHINPurposeOfUse {
    fn contains(v: &Code) -> bool {
        Self::contains_code(v)
    }
}
impl super::super::bindings::ValueSetMembership<Coding> for NHINPurposeOfUse {
    fn contains(v: &Coding) -> bool {
        Self::contains_coding(v)
    }
}
impl super::super::bindings::ValueSetMembership<CodeableConcept> for NHINPurposeOfUse {
    fn contains(v: &CodeableConcept) -> bool {
        Self::contains_codeable_concept(v)
    }
}
fn is_rgb_hex(code: &str) -> bool {
    let b = code.as_bytes();
    if b.len() != 7 || b[0] != b'#' {
        return false;
    }
    fn is_hex(x: u8) -> bool {
        (b'0'..=b'9').contains(&x) || (b'a'..=b'f').contains(&x)
            || (b'A'..=b'F').contains(&x)
    }
    is_hex(b[1]) && is_hex(b[2]) && is_hex(b[3]) && is_hex(b[4]) && is_hex(b[5])
        && is_hex(b[6])
}

