...
///Canonical URL: http://hl7.org/fhir/action-condition-kind
///Version: 5.0.0
...
impl core::convert::TryFrom<&str> for ActionConditionKind {
    type Error = ();
    fn try_from(s: &str) -> Result<Self, <Self as core::convert::TryFrom<&str>>::Error> {
        match s {
            "applicability" => Ok(ActionConditionKind::Applicability),
            "start" => Ok(ActionConditionKind::Start),
            "stop" => Ok(ActionConditionKind::Stop),
            _ => Err(()),
        }
    }
}
impl core::fmt::Display for ActionConditionKind {
