...
///Canonical URL: http://hl7.org/fhir/action-required-behavior
///Version: 5.0.0
...
impl core::convert::TryFrom<&str> for ActionRequiredBehavior {
    type Error = ();
    fn try_from(s: &str) -> Result<Self, <Self as core::convert::TryFrom<&str>>::Error> {
        match s {
            "must" => Ok(ActionRequiredBehavior::Must),
            "could" => Ok(ActionRequiredBehavior::Could),
            "must-unless-documented" => Ok(ActionRequiredBehavior::MustUnlessDocumented),
            _ => Err(()),
        }
    }
}
impl core::fmt::Display for ActionRequiredBehavior {
