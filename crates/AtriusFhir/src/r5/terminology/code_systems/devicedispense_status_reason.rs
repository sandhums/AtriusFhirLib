// @generated by AtriusValueSetGen
// DO NOT EDIT MANUALLY

#![allow(non_camel_case_types)]
#![allow(non_snake_case)]
#![allow(clippy::upper_case_acronyms)]

use serde::{Deserialize, Serialize};

use super::super::super::{Boolean, Code, CodeableConcept, Coding, Element, Uri};

use super::super::super::string::String as FhirString;

///FHIR CodeSystem: DeviceDispenseStatusReasonCodes
///Canonical URL: http://hl7.org/fhir/CodeSystem/devicedispense-status-reason
///Version: 5.0.0
///Title: DeviceDispense Status Reason Codes
///Status: active
///DeviceDispense Status Reason Codes
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub enum DeviceDispenseStatusReasonCodes {
    ///Display: Out of Stock
    ///Definition: The device was not dispensed because it was not available.
    #[serde(rename = "out-of-stock")]
    OutOfStock,
    ///Display: Off market
    ///Definition: The device was not dispensed because it is of-market - for example not authorized, withdrawn or recalled.
    #[serde(rename = "off-market")]
    OffMarket,
    ///Display: Contraindication
    ///Definition: The device was not dispensed because a contraindication was found - for example pregnancy, allergy to a device component...
    #[serde(rename = "contraindication")]
    Contraindication,
    ///Display: Incompatible device
    ///Definition: The device was not dispensed because an incompatibility has been found with the device or between the device and other devices being used in the same context.
    #[serde(rename = "incompatible-device")]
    IncompatibleDevice,
    ///Display: Order expired
    ///Definition: The device was not dispensed because the order has expired or been invalidated.
    #[serde(rename = "order-expired")]
    OrderExpired,
    ///Display: Verbal order
    ///Definition: The device not dispensed because there was a verbal order.
    #[serde(rename = "verbal-order")]
    VerbalOrder,
}
impl DeviceDispenseStatusReasonCodes {
    pub fn as_code(&self) -> &'static str {
        match self {
            DeviceDispenseStatusReasonCodes::OutOfStock => "out-of-stock",
            DeviceDispenseStatusReasonCodes::OffMarket => "off-market",
            DeviceDispenseStatusReasonCodes::Contraindication => "contraindication",
            DeviceDispenseStatusReasonCodes::IncompatibleDevice => "incompatible-device",
            DeviceDispenseStatusReasonCodes::OrderExpired => "order-expired",
            DeviceDispenseStatusReasonCodes::VerbalOrder => "verbal-order",
        }
    }
    pub fn system() -> &'static str {
        "http://hl7.org/fhir/CodeSystem/devicedispense-status-reason"
    }
    pub fn version() -> Option<&'static str> {
        Some("5.0.0")
    }
    /// Convert this code into a FHIR `code` primitive (`Code = Element<String, Extension>`).
    ///
    /// Useful for elements like `Patient.gender` that are bound directly to a ValueSet and use
    /// the `code` datatype.
    pub fn to_code(self) -> Code {
        Element {
            id: None,
            extension: None,
            value: Some(self.as_code().to_string()),
        }
    }
    /// Convert this code into a FHIR `Coding` with `system` and (if available) `version` set.
    ///
    /// Useful for `Coding` fields and for inclusion inside `CodeableConcept.coding`.
    pub fn to_coding(self) -> Coding {
        let system: Uri = Element {
            id: None,
            extension: None,
            value: Some(Self::system().to_string()),
        };
        Coding {
            id: None,
            extension: None,
            system: Some(system),
            version: Self::version()
                .map(|v| FhirString {
                    id: None,
                    extension: None,
                    value: Some(v.to_string()),
                }),
            code: Some(self.to_code()),
            display: None,
            user_selected: Option::<Boolean>::None,
        }
    }
    /// Convert this code into a minimal `CodeableConcept` (with a single `coding`).
    ///
    /// This is especially useful for bound elements that use `CodeableConcept` (e.g.
    /// extensible bindings).
    pub fn to_codeable_concept(self) -> CodeableConcept {
        CodeableConcept {
            id: None,
            extension: None,
            coding: Some(vec![self.to_coding()]),
            text: None,
        }
    }
    /// Parse a code string into this enum.
    ///
    /// This is a convenience wrapper around the `TryFrom<&str>` implementation.
    pub fn try_from_code(code: &str) -> Result<Self, ()> {
        <Self as core::convert::TryFrom<&str>>::try_from(code)
    }
    /// Parse from a FHIR `code` primitive (`Code = Element<String, Extension>`).
    pub fn try_from_code_element(code: &Code) -> Result<Self, ()> {
        match code.value.as_deref() {
            Some(v) => Self::try_from_code(v),
            None => Err(()),
        }
    }
}
impl core::convert::TryFrom<&str> for DeviceDispenseStatusReasonCodes {
    type Error = ();
    fn try_from(s: &str) -> Result<Self, <Self as core::convert::TryFrom<&str>>::Error> {
        match s {
            "out-of-stock" => Ok(DeviceDispenseStatusReasonCodes::OutOfStock),
            "off-market" => Ok(DeviceDispenseStatusReasonCodes::OffMarket),
            "contraindication" => Ok(DeviceDispenseStatusReasonCodes::Contraindication),
            "incompatible-device" => {
                Ok(DeviceDispenseStatusReasonCodes::IncompatibleDevice)
            }
            "order-expired" => Ok(DeviceDispenseStatusReasonCodes::OrderExpired),
            "verbal-order" => Ok(DeviceDispenseStatusReasonCodes::VerbalOrder),
            _ => Err(()),
        }
    }
}
impl core::fmt::Display for DeviceDispenseStatusReasonCodes {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.write_str(self.as_code())
    }
}

