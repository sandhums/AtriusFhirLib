...
///Canonical URL: http://hl7.org/fhir/document-reference-status
///Version: 5.0.0
...
impl core::convert::TryFrom<&str> for DocumentReferenceStatus {
    type Error = ();
    fn try_from(s: &str) -> Result<Self, <Self as core::convert::TryFrom<&str>>::Error> {
        match s {
            "current" => Ok(DocumentReferenceStatus::Current),
            "superseded" => Ok(DocumentReferenceStatus::Superseded),
            "entered-in-error" => Ok(DocumentReferenceStatus::EnteredInError),
            _ => Err(()),
        }
    }
}
impl core::fmt::Display for DocumentReferenceStatus {
