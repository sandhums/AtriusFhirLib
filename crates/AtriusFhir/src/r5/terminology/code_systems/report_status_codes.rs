...
///FHIR CodeSystem: TestReportStatus
///Canonical URL: http://hl7.org/fhir/report-status-codes
///Version: 5.0.0
...
impl core::convert::TryFrom<&str> for TestReportStatus {
    type Error = ();
    fn try_from(s: &str) -> Result<Self, <Self as core::convert::TryFrom<&str>>::Error> {
        match s {
            "completed" => Ok(TestReportStatus::Completed),
            "in-progress" => Ok(TestReportStatus::InProgress),
            "waiting" => Ok(TestReportStatus::Waiting),
            "stopped" => Ok(TestReportStatus::Stopped),
            "entered-in-error" => Ok(TestReportStatus::EnteredInError),
            _ => Err(()),
        }
    }
}
impl core::fmt::Display for TestReportStatus {
