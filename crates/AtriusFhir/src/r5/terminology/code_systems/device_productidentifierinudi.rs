// @generated by AtriusValueSetGen
// DO NOT EDIT MANUALLY

#![allow(non_camel_case_types)]
#![allow(non_snake_case)]
#![allow(clippy::upper_case_acronyms)]

use serde::{Deserialize, Serialize};

use super::super::super::{Boolean, Code, CodeableConcept, Coding, Element, Uri};

use super::super::super::string::String as FhirString;

///FHIR CodeSystem: DeviceProductionIdentifierInUDI
///Canonical URL: http://hl7.org/fhir/device-productidentifierinudi
///Version: 5.0.0
///Title: Device Production Identifier In UDI
///Status: active
///The production identifier(s) that are expected to appear in the UDI carrier.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub enum DeviceProductionIdentifierInUDI {
    ///Display: Lot Number
    ///Definition: The label includes the lot number.
    #[serde(rename = "lot-number")]
    LotNumber,
    ///Display: Manufactured date
    ///Definition: The label includes the manufacture date.
    #[serde(rename = "manufactured-date")]
    ManufacturedDate,
    ///Display: Serial Number
    ///Definition: The label includes the serial number.
    #[serde(rename = "serial-number")]
    SerialNumber,
    ///Display: Expiration date
    ///Definition: The label includes the expiration date.
    #[serde(rename = "expiration-date")]
    ExpirationDate,
    ///Display: Biological source
    ///Definition: The label includes the biological source identifier.
    #[serde(rename = "biological-source")]
    BiologicalSource,
    ///Display: Software Version
    ///Definition: The label includes the software version.
    #[serde(rename = "software-version")]
    SoftwareVersion,
}
impl DeviceProductionIdentifierInUDI {
    pub fn as_code(&self) -> &'static str {
        match self {
            DeviceProductionIdentifierInUDI::LotNumber => "lot-number",
            DeviceProductionIdentifierInUDI::ManufacturedDate => "manufactured-date",
            DeviceProductionIdentifierInUDI::SerialNumber => "serial-number",
            DeviceProductionIdentifierInUDI::ExpirationDate => "expiration-date",
            DeviceProductionIdentifierInUDI::BiologicalSource => "biological-source",
            DeviceProductionIdentifierInUDI::SoftwareVersion => "software-version",
        }
    }
    pub fn system() -> &'static str {
        "http://hl7.org/fhir/device-productidentifierinudi"
    }
    pub fn version() -> Option<&'static str> {
        Some("5.0.0")
    }
    /// Convert this code into a FHIR `code` primitive (`Code = Element<String, Extension>`).
    ///
    /// Useful for elements like `Patient.gender` that are bound directly to a ValueSet and use
    /// the `code` datatype.
    pub fn to_code(self) -> Code {
        Element {
            id: None,
            extension: None,
            value: Some(self.as_code().to_string()),
        }
    }
    /// Convert this code into a FHIR `Coding` with `system` and (if available) `version` set.
    ///
    /// Useful for `Coding` fields and for inclusion inside `CodeableConcept.coding`.
    pub fn to_coding(self) -> Coding {
        let system: Uri = Element {
            id: None,
            extension: None,
            value: Some(Self::system().to_string()),
        };
        Coding {
            id: None,
            extension: None,
            system: Some(system),
            version: Self::version()
                .map(|v| FhirString {
                    id: None,
                    extension: None,
                    value: Some(v.to_string()),
                }),
            code: Some(self.to_code()),
            display: None,
            user_selected: Option::<Boolean>::None,
        }
    }
    /// Convert this code into a minimal `CodeableConcept` (with a single `coding`).
    ///
    /// This is especially useful for bound elements that use `CodeableConcept` (e.g.
    /// extensible bindings).
    pub fn to_codeable_concept(self) -> CodeableConcept {
        CodeableConcept {
            id: None,
            extension: None,
            coding: Some(vec![self.to_coding()]),
            text: None,
        }
    }
    /// Parse a code string into this enum.
    ///
    /// This is a convenience wrapper around the `TryFrom<&str>` implementation.
    pub fn try_from_code(code: &str) -> Result<Self, ()> {
        <Self as core::convert::TryFrom<&str>>::try_from(code)
    }
    /// Parse from a FHIR `code` primitive (`Code = Element<String, Extension>`).
    pub fn try_from_code_element(code: &Code) -> Result<Self, ()> {
        match code.value.as_deref() {
            Some(v) => Self::try_from_code(v),
            None => Err(()),
        }
    }
}
impl core::convert::TryFrom<&str> for DeviceProductionIdentifierInUDI {
    type Error = ();
    fn try_from(s: &str) -> Result<Self, <Self as core::convert::TryFrom<&str>>::Error> {
        match s {
            "lot-number" => Ok(DeviceProductionIdentifierInUDI::LotNumber),
            "manufactured-date" => Ok(DeviceProductionIdentifierInUDI::ManufacturedDate),
            "serial-number" => Ok(DeviceProductionIdentifierInUDI::SerialNumber),
            "expiration-date" => Ok(DeviceProductionIdentifierInUDI::ExpirationDate),
            "biological-source" => Ok(DeviceProductionIdentifierInUDI::BiologicalSource),
            "software-version" => Ok(DeviceProductionIdentifierInUDI::SoftwareVersion),
            _ => Err(()),
        }
    }
}
impl core::fmt::Display for DeviceProductionIdentifierInUDI {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.write_str(self.as_code())
    }
}

