...
///FHIR CodeSystem: ProductContactType
///Canonical URL: http://hl7.org/fhir/medicinal-product-contact-type
///Version: 5.0.0
...
impl core::convert::TryFrom<&str> for ProductContactType {
    type Error = ();
    fn try_from(s: &str) -> Result<Self, <Self as core::convert::TryFrom<&str>>::Error> {
        match s {
            "ProposedMAH" => Ok(ProductContactType::Proposedmah),
            "ProcedureContactDuring" => Ok(ProductContactType::Procedurecontactduring),
            "ProcedureContactAfter" => Ok(ProductContactType::Procedurecontactafter),
            "QPPV" => Ok(ProductContactType::Qppv),
            "PVEnquiries" => Ok(ProductContactType::Pvenquiries),
            _ => Err(()),
        }
    }
}
impl core::fmt::Display for ProductContactType {
