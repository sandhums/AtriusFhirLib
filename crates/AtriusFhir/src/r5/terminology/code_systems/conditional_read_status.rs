...
///Canonical URL: http://hl7.org/fhir/conditional-read-status
///Version: 5.0.0
...
impl ConditionalReadStatus {
    pub fn as_code(&self) -> &'static str {
        match self {
            ConditionalReadStatus::NotSupported => "not-supported",
            ConditionalReadStatus::ModifiedSince => "modified-since",
            ConditionalReadStatus::NotMatch => "not-match",
            ConditionalReadStatus::FullSupport => "full-support",
        }
    }
...
impl core::convert::TryFrom<&str> for ConditionalReadStatus {
    type Error = ();
    fn try_from(s: &str) -> Result<Self, <Self as core::convert::TryFrom<&str>>::Error> {
        match s {
            "not-supported" => Ok(ConditionalReadStatus::NotSupported),
            "modified-since" => Ok(ConditionalReadStatus::ModifiedSince),
            "not-match" => Ok(ConditionalReadStatus::NotMatch),
            "full-support" => Ok(ConditionalReadStatus::FullSupport),
            _ => Err(()),
        }
    }
}
impl core::fmt::Display for ConditionalReadStatus {
