...
///Canonical URL: http://hl7.org/fhir/explanationofbenefit-status
///Version: 5.0.0
...
impl core::convert::TryFrom<&str> for ExplanationOfBenefitStatus {
    type Error = ();
    fn try_from(s: &str) -> Result<Self, <Self as core::convert::TryFrom<&str>>::Error> {
        match s {
            "active" => Ok(ExplanationOfBenefitStatus::Active),
            "cancelled" => Ok(ExplanationOfBenefitStatus::Cancelled),
            "draft" => Ok(ExplanationOfBenefitStatus::Draft),
            "entered-in-error" => Ok(ExplanationOfBenefitStatus::EnteredInError),
            _ => Err(()),
        }
    }
}
impl core::fmt::Display for ExplanationOfBenefitStatus {
