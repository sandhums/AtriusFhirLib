...
///Canonical URL: http://hl7.org/fhir/measure-report-status
///Version: 5.0.0
...
impl core::convert::TryFrom<&str> for MeasureReportStatus {
    type Error = ();
    fn try_from(s: &str) -> Result<Self, <Self as core::convert::TryFrom<&str>>::Error> {
        match s {
            "complete" => Ok(MeasureReportStatus::Complete),
            "pending" => Ok(MeasureReportStatus::Pending),
            "error" => Ok(MeasureReportStatus::Error),
            _ => Err(()),
        }
    }
}
impl core::fmt::Display for MeasureReportStatus {
