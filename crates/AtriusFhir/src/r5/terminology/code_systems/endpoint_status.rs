...
///Canonical URL: http://hl7.org/fhir/endpoint-status
///Version: 5.0.0
...
impl core::convert::TryFrom<&str> for EndpointStatus {
    type Error = ();
    fn try_from(s: &str) -> Result<Self, <Self as core::convert::TryFrom<&str>>::Error> {
        match s {
            "active" => Ok(EndpointStatus::Active),
            "suspended" => Ok(EndpointStatus::Suspended),
            "error" => Ok(EndpointStatus::Error),
            "off" => Ok(EndpointStatus::Off),
            "entered-in-error" => Ok(EndpointStatus::EnteredInError),
            _ => Err(()),
        }
    }
}
impl core::fmt::Display for EndpointStatus {
