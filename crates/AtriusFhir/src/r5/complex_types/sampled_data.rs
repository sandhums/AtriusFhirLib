...
#[fhir_invariant(key="sdd-1", severity="error", human="A SampledData SAHLL have either an interval and offsets but not both", expr="interval.exists().not() xor offsets.exists().not()", path="SampledData")]
