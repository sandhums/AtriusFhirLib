...
pub struct DataRequirement {
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children", expr="hasValue() or (children().count() > id.count())", path="DataRequirement.valueFilter")]
...
pub struct DataRequirementValueFilter {
    ...
    #[fhir_invariant(key="ext-1", severity="error", human="Must have either extensions or value[x], not both", expr="extension.exists() != value.exists()", path="DataRequirement.valueFilter.extension")]
    pub extension: Option<Vec<Extension>>,
