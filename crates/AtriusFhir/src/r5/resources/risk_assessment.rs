...
pub struct RiskAssessment {
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children", expr="hasValue() or (children().count() > id.count())", path="RiskAssessment.reason")]
    ...
    #[fhir_invariant(key="ras-2", severity="error", human="Probability as a deciml must be <= 100", expr="probability.empty() or ((probability is decimal) implies ((probability as decimal) <= 100))", path="RiskAssessment.prediction")]
