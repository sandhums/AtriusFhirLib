...
pub struct CareTeam {
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    #[fhir_invariant(key="ctm-1", severity="error", human="CareTeam.participant.onBehalfOf can only be populated when CareTeam.participant.member is a Practitioner", expr="onBehalfOf.exists() implies (member.resolve() is Practitioner)", path="CareTeam.participant")]
    #[fhir_invariant(key="ctm-2", severity="warning", human="CareTeam.participant.role or CareTeam.participant.member exists", expr="role.exists() or member.exists()", path="CareTeam.participant")]
...
pub struct CareTeamParticipant {
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children", expr="hasValue() or (children().count() > id.count())", path="CareTeam.participant.coverage[x]")]
    #[fhir_serde(flatten)]
