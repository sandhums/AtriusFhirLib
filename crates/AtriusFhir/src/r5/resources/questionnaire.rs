...
pub struct Questionnaire {
    ...
    #[fhir_invariant(key="que-5", severity="error", human="Only coding, decimal, integer, date, dateTime, time, string or quantity  items can have answerOption or answerValueSet", expr="(type='coding' or type = 'decimal' or type = 'integer' or type = 'date' or type = 'dateTime' or type = 'time' or type = 'string' or type = 'quantity') or (answerValueSet.empty() and answerOption.empty())", path="Questionnaire.item")]
    ...
    #[fhir_invariant(key="que-10", severity="error", human="Maximum length can only be declared for simple question types", expr="(type in ('boolean' | 'decimal' | 'integer' | 'string' | 'text' | 'url')) or answerConstraint='optionOrString' or maxLength.empty()", path="Questionnaire.item")]
