...
pub struct RiskAssessment {
    ...
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children or both", expr="hasValue() or (children().count() > id.count())", path="RiskAssessment.prediction")]
    #[fhir_invariant(key="ras-2", severity="error", human="Probability as a decimal must be <= 100", expr="probability.empty() or ((probability is decimal) implies ((probability as decimal) <= 100))", path="RiskAssessment.prediction")]
    #[fhir_invariant(key="ras-3", severity="error", human="Probability as a Quantity must be <= 100%", expr="probability.empty() or ((probability is Quantity) implies (((probability as Quantity).value <= 100) and ((probability as Quantity).code = '%') and ((probability as Quantity).system = %ucum)))", path="RiskAssessment.prediction")]
