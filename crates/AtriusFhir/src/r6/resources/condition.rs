...
#[fhir_invariant(key="con-4", severity="error", human="bodyStructure SHALL only be present if Condition.bodySite is not present", expr="bodySite.exists() implies bodyStructure.empty()", path="Condition")]
...
pub struct Condition {
    ...
    /// ## Conditions
    /// Used when: con-4
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children", expr="hasValue() or (children().count() > id.count())", path="Condition.bodyStructure")]
    ...
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children or both", expr="hasValue() or (children().count() > id.count())", path="Condition.stage")]
