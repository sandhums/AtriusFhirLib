...
pub struct NamingSystem {
    ...
    #[fhir_invariant(key="nsd-4", severity="error", human="Related artifact can't be depends-on or composed-of", expr="(type in ('depends-on' | 'composed-of')).not()", path="NamingSystem.relatedArtifact")]
    ...
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children or both", expr="hasValue() or (children().count() > id.count())", path="NamingSystem.uniqueId")]
