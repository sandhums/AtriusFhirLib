...
#[fhir_invariant(key="spd-4", severity="error", human="Search parameter codes SHALL NOT contain '.', '$', '|', or whitespace characters", expr="code.matches('^[^.\\\\$\\\\|\\\\s]+$')", path="SearchParameter")]
#[fhir_invariant(key="spd-5", severity="error", human="Search parameter aliases SHALL NOT contain '.', '$', '|', or whitespace characters", expr="aliasCode.empty() or aliasCode.all(matches('^[^.\\\\$\\\\|\\\\s]+$')).allTrue()", path="SearchParameter")]
pub struct SearchParameter {
    ...
    /// ## Conditions
    /// Used when: spd-5
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children", expr="hasValue() or (children().count() > id.count())", path="SearchParameter.aliasCode")]
    ...
    ///   Expression: `hasValue() or (children().count() > id.count())`
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children or both", expr="hasValue() or (children().count() > id.count())", path="SearchParameter.component")]
