...
#[fhir_invariant(key="cnl-0", severity="warning", human="Name should be usable as an identifier for the module by machine processing applications such as code generation", expr="name.exists() implies name.matches('^[A-Z]([A-Za-z0-9_]){1,254}$')", path="Questionnaire")]
#[fhir_invariant(key="que-2", severity="error", human="The link ids for groups and questions must be unique within the questionnaire", expr="repeat(item).linkId.isDistinct()", path="Questionnaire")]
pub struct Questionnaire {
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    #[fhir_invariant(key="cnl-1", severity="warning", human="URL should not contain | or # - these characters make processing canonical references problematic", expr="exists() implies matches('^[^|# ]+$')", path="Questionnaire.url")]
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children", expr="hasValue() or (children().count() > id.count())", path="Questionnaire.copyrightLabel")]
    ...
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children or both", expr="hasValue() or (children().count() > id.count())", path="Questionnaire.item")]
    #[fhir_invariant(key="que-12", severity="error", human="If there are more than one enableWhen, enableBehavior must be specified", expr="enableWhen.count() > 1 implies enableBehavior.exists()", path="Questionnaire.item")]
    #[fhir_invariant(key="que-18a", severity="error", human="The types of any specified answerOptions must align with the item type", expr="answerOption.value.exists() implies defineVariable('type', type).trace('type', %type).answerOption.value.type().name.distinct().select(iif($this='simplequantity', 'quantity', $this.lower())).where(%type!=$this and (%type='text' and $this='string').not() and (%type='url' and $this='uri').not()).trace('Unmatched answerOption type(s)').empty()", path="Questionnaire.item")]
    #[fhir_invariant(key="que-18b", severity="error", human="The types of any specified initial values must align with the item type", expr="initial.value.exists() implies defineVariable('type', type).trace('type', %type).initial.value.type().name.distinct().select(iif($this='simplequantity', 'quantity', $this.lower())).where(%type!=$this and (%type='text' and $this='string').not() and (%type='url' and $this='uri').not()).trace('Unmatched initial value type(s)').empty()", path="Questionnaire.item")]
    #[fhir_invariant(key="que-1a", severity="error", human="Group items must have nested items when Questionanire is active", expr="(type='group' and %resource.status='active') implies item.empty().not()", path="Questionnaire.item")]
    #[fhir_invariant(key="que-1b", severity="warning", human="Groups should have items", expr="type='group' implies item.empty().not()", path="Questionnaire.item")]
    #[fhir_invariant(key="que-1c", severity="error", human="Display items cannot have child items", expr="type='display' implies item.empty()", path="Questionnaire.item")]
    ...
    #[fhir_invariant(key="que-5", severity="error", human="Only coding, decimal, integer, date, dateTime, time, string, quantity, reference, or uri items can have answerOption", expr="(('coding' | 'decimal' | 'integer' | 'date' | 'dateTime' | 'time' | 'string' | 'quantity' | 'reference' | 'uri') contains type) or answerOption.empty()", path="Questionnaire.item")]
    #[fhir_invariant(key="que-5b", severity="error", human="Only coding, decimal, integer, date, dateTime, time, string or quantity  items can have answerOption", expr="(('coding' | 'string' | 'uri') contains type) or answerValueSet.empty()", path="Questionnaire.item")]
    ...
    #[fhir_invariant(key="que-10", severity="error", human="Maximum length can only be declared for simple question types", expr="(type in ('string' | 'decimal' | 'integer' | 'text' | 'date' | 'dateTime' | 'time' | 'url')) or answerConstraint='optionOrString' or maxLength.empty()", path="Questionnaire.item")]
    #[fhir_invariant(key="que-11", severity="error", human="If one or more answerOption is present, initial cannot be present.  Use answerOption.initialSelected instead", expr="answerOption.empty() or initial.empty()", path="Questionnaire.item")]
    ...
    #[fhir_invariant(key="que-14", severity="warning", human="Can only have answerConstraint if answerOption or answerValueSet are present.  (This is a warning because extensions may serve the same purpose)", expr="answerConstraint.exists() implies answerOption.exists() or answerValueSet.exists()", path="Questionnaire.item")]
    #[fhir_invariant(key="que-17", severity="error", human="If question is non-repeating, cannot have more than one initialSelected answer option", expr="repeats.not() implies answerOption.where(initialSelected=true).count() <= 1", path="Questionnaire.item")]
...
pub struct QuestionnaireItem {
    ...
    #[fhir_invariant(key="que-15", severity="warning", human="Link ids should be 255 characters or less", expr="$this.length() <= 255", path="Questionnaire.item.linkId")]
    #[fhir_invariant(key="que-16", severity="error", human="LinkId cannot have leading or trailing spaces or intevening whitespace other than single space characters", expr="$this.matches('[^\\\\s]+( [^\\\\s]+)*')", path="Questionnaire.item.linkId")]
    ...
    /// ## Conditions
    /// Used when: que-12
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children or both", expr="hasValue() or (children().count() > id.count())", path="Questionnaire.item.enableWhen")]
    ...
    #[fhir_invariant(key="que-18", severity="warning", human="Referenced questions SHOULD exist in the Questionnaire (unless still in draft, working with modular forms, etc.)", expr="question.exists() implies (%resource.repeat(item).linkId contains question)", path="Questionnaire.item.enableWhen")]
    ...
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children or both", expr="hasValue() or (children().count() > id.count())", path="Questionnaire.item.answerOption")]
    ...
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children or both", expr="hasValue() or (children().count() > id.count())", path="Questionnaire.item.initial")]
