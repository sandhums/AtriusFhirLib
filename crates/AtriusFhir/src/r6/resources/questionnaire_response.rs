...
pub struct QuestionnaireResponse {
    ...
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children or both", expr="hasValue() or (children().count() > id.count())", path="QuestionnaireResponse.item")]
...
pub struct QuestionnaireResponseItem {
    ...
    #[fhir_invariant(key="qrs-3", severity="error", human="LinkId cannot have leading or trailing spaces or intevening whitespace other than single space characters", expr="$this.matches('[^\\\\s]+( [^\\\\s]+)*')", path="QuestionnaireResponse.item.linkId")]
    ...
    #[fhir_invariant(key="ele-1", severity="error", human="All FHIR elements must have a @value or children or both", expr="hasValue() or (children().count() > id.count())", path="QuestionnaireResponse.item.answer")]
